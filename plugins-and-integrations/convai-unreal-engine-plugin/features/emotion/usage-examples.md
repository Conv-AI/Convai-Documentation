...
**Situation:** A scripted scene requires the character to express grief at a specific moment, regardless of what the server sends.

### Blueprint steps

1. Get a reference to the `Convai Chatbot` component on the character.
2. Call **Force Set Emotion**. Set **Basic Emotion** to `Sad`, **Intensity** to `More Intense`, and **Reset Other Emotions** to `true`. The score applied will be `1.0` (the MoreIntense multiplier).
3. `On Emotion State Changed` fires immediately after the call completes. If your handler is already wired to apply blendshapes, the face updates automatically. If not, call `Get Emotion Blendshapes` on the same component and apply the map directly.
...
**Situation:** A cutscene must hold a character at an angry expression while the server continues sending conversation-driven updates that should be suppressed.
...
2. Set `Lock Emotion State` to `true` on the `Convai Chatbot` component. All server emotion updates arriving during the cutscene are silently discarded.
3. Apply the blendshapes from `Get Emotion Blendshapes` to the mesh if they are not already applied by your event handler.
...
**Situation:** A particle system around the character should intensify as the character's fear score rises — for example, a visual aura that signals distress.

### Blueprint steps

1. Subscribe to `On Emotion State Changed` in your character Blueprint.
2. In the event handler, call `Get Emotion Score` with **Emotion** set to `Fear`. The return value is a `float` in `0.0`–`1.0`.
...
1. Call `Reset Emotion State` on the `Convai Chatbot` component. This zeros all scores and clears the blendshape map. `On Emotion State Changed` fires immediately after.
2. If your event handler does not automatically re-apply the cleared blendshape map, call `Get Emotion Blendshapes` after the reset — the returned map will have all weights at `0.0`.
3. Apply the zero-weight map to the mesh using `For Each Loop (Map)` + `Set Morph Target`. The character's face returns to its rest pose.
...
5. Compile and test in Play mode. The character blends toward the surprised pose as the `Surprise` score rises.
