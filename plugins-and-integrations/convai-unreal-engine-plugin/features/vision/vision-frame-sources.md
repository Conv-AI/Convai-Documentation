...
If you are setting up vision for the first time, complete [Vision quick start](vision-quick-start.md) first. Use this page to configure frame sources after the initial setup.
...
- Follow the **Event BeginPlay → Start** pattern from [Vision quick start](vision-quick-start.md).
...
Assign **Convai Render Target** and call **Start** as described in [Vision quick start](vision-quick-start.md). The chatbot receives frames of the webcam image because that image is now part of the captured scene.
{% endstep %}
