...
- The Convai Unreal Engine plugin is installed and the API key is configured. See [Install the Convai plugin](../../getting-started/installation.md).
...
If the highlight never appears, see [Troubleshoot gaze attention](troubleshooting-and-diagnostics.md). If the highlight appears but the character stays silent, confirm **Enable Actions**, `GazeShouldRespond`, and `GazeAttentionText` on the player component.
