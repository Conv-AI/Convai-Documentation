...
The `ObjectEntry` property (`FConvaiObjectEntry`, category `Convai|Object`) describes the `Actor` to Convai. Because `ObjectEntry` uses `ShowOnlyInnerProperties`, its fields appear directly in the Details panel rather than under a collapsed struct.
