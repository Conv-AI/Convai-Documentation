...
{% hint style="info" %}
Command-line overrides (`-LipSyncAnim*` flags) set during PIE do not carry over to packaged builds unless passed explicitly at launch. See the [Face Sync AnimGraph node reference](face-sync-animgraph-node-reference.md) for the full list of supported override flags.
{% endhint %}
