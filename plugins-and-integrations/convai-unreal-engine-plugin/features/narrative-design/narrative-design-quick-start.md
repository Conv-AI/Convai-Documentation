...
- A character `Actor` Blueprint with a `UConvaiChatbotComponent` (`Convai Chatbot`) and `CharacterID` set to a valid ID. See [Add your first Convai character](../../getting-started/add-your-first-convai-character.md).
...
If nothing prints, see [Troubleshoot narrative design](troubleshoot-narrative-design.md). Common causes: trigger name mismatch, wrong `CharacterID`, or a trigger not connected to the current section in the dashboard.
{% endstep %}
{% endstepper %}
