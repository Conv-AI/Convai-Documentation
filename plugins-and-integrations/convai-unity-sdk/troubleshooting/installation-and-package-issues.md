...
If Unity enters an infinite recompile loop after installing the package, close Unity, delete the `Library/` folder, and reopen. The Library cache is rebuilt on next launch.
...
| Package not found when adding via UPM name                  | Scoped registry not configured                  | Follow the UPM installation guide to add the Convai scoped registry to your project's `manifest.json`                             |
...
| Scene sample imports correctly but does not run             | URP package missing                             | Sample scenes require URP; install `com.unity.render-pipelines.universal` and assign the URP asset in Project Settings → Graphics |
...
Once the SDK initializes cleanly and the bootstrapper shows `Initialization complete.`, the next issue category to check is connection and API key validation.

{% content-ref url="/broken/pages/5327e30e65a2bff2e207d1147c66230ab0a66b57" %}
[Broken link](/broken/pages/5327e30e65a2bff2e207d1147c66230ab0a66b57)
{% endcontent-ref %}
