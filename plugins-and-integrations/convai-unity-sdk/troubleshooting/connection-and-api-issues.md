...
Every connection failure in the Convai Unity SDK produces a structured error code. Understanding what each code means — and whether the SDK will retry automatically — is the fastest path to a fix. This page covers all connection and API error codes, explains the retry system, and shows how to read live diagnostic state at runtime.
...
{% hint style="warning" %}
The SDK also emits a warning before any connect attempt if the API key is empty: `Convai Bootstrapper: API key not configured. Please set your API key in Edit > Project Settings > Convai SDK.` This fires on Play — fix it before testing connections.
{% endhint %}
...
These codes appear when the Convai API rejects or cannot fulfill the connect request. Most have a clear cause and a direct fix.
...
Transport errors occur at the WebRTC / LiveKit layer, after the Convai API has accepted the connect request. They are almost always transient and the SDK retries automatically.
...
{% hint style="info" %}
WebRTC requires that UDP ports are not strictly blocked. In corporate or enterprise training simulation deployments, work with your network team to whitelist the Convai LiveKit endpoints. See the Network & API Requirements page for the full list of domains and ports.
{% endhint %}
...
| ------------------------- | -------------- | ------------------------------------------------------------------------------------------------- |
| `CurrentState`            | `SessionState` | Current session state: `Disconnected`, `Connecting`, `Connected`, `Disconnecting`, `Reconnecting` |
...
{% code title="DiagnosticsLogger.cs" %}
```csharp
...
For a full reference to the SDK's logging system, built-in diagnostic tools, and session metrics, see the Debug Tools Reference page.
