...
| Rig section reads `Needs Attention` with a low-confidence message | `DetectionConfidence` is below `50%` | Check that expression and lip sync look correct; if not, set the convention manually instead of trusting auto-detection | `DetectionConfidence` reported by `StandardRigBinding` rises above `50%`, or the convention is set manually |
