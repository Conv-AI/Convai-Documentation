...
With the taxonomy in place, the pipeline has a complete, authoritative mapping from server labels to the canonical identifiers that drive your output bindings. Continue to [Scripting API](../../../unity-plugin-beta-overview/features/emotion/scripting-api-reference.md) to learn how to read and control emotion state from C# at runtime, or see [Troubleshooting](../../../unity-plugin-beta-overview/features/emotion/troubleshooting-and-diagnostics.md) if taxonomy warning messages are appearing in the Console.
