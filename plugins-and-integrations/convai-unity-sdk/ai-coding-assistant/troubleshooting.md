...
|---|---|---|---|
| `Convai MCP Tools` row shows `<count>/20; install Unity AI Assistant first` | The Unity AI Assistant package isn't installed, or its version is outside Convai's accepted range | Click `Fix` on the `Unity AI Assistant` row first; the tools repair only becomes available once the Assistant is ready | The `Unity AI Assistant` row shows a ready check mark, then the `Convai MCP Tools` repair becomes available |
