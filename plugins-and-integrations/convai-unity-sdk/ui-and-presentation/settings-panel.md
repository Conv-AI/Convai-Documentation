...
The settings panel is an optional scene-level UI that lets users adjust key conversation preferences at runtime. It is built using the MVP pattern: `SettingsPanel` handles rendering, `SettingsPanelPresenter` manages logic, and `IConvaiSettingsPanelController` owns visibility state. All three wire together automatically when the prefab is in the scene.

The underlying settings service — `IConvaiRuntimeSettingsService` — is also a standalone scripting API. You can read current settings, apply patches, and subscribe to changes entirely from code, without the panel being present at all.
...
## Adding the Settings Panel to Your Scene

{% stepper %}
{% step %}
**Add the Prefab to Your Canvas**

Drag `Packages/com.convai.convai-sdk-for-unity/Prefabs/SettingsPanel/SettingsPanel_Landscape.prefab` into your scene's Canvas hierarchy.
...
| `Snapshot`          | `ConvaiRuntimeSettingsSnapshot`   | Resulting settings after apply                               |
...
**`ResetToDefaults()`** resets all settings to project defaults and clears all overrides:
...
{
    // Check what changed using the bitmask before doing work
...
See [Chat and Subtitle Modes](/broken/pages/7a60234ac501c2d7c4f7240396d285b464a3ae06) for full mode-switching details.

***

## Usage Examples

### Training Simulation — Pause Menu Integration

A safety training simulation exposes the settings panel through a pause menu and re-shows the menu when the panel closes:

```csharp
using Convai.Domain.Shared.Abstractions;
using Convai.Runtime.Components;
using UnityEngine;
...
### Onboarding — Pre-Populate Settings from Authentication
