...
Add at least one `EmotionSlotBinding` with a valid `emotionLabel` and a blendshape name or Animator parameter that exists on your character. See [Output Bindings](/broken/pages/1e2b2eb9321a550a8ea8a49276a5aaad36013bb2).
{% endstep %}
...
**Fix:** Open your custom taxonomy asset (or create one if using the built-in default), and add the server label as an alias to the nearest semantic match. For example, if the server sends `"excited"` and it should map to the visual slot for `"anticipation"`, add `"excited"` to the **Aliases** list of the `anticipation` entry. See [Emotion Taxonomy](/broken/pages/d16937e3d6537821e2c4af08d397101d0111d1ee) for how to create and assign a custom taxonomy.
