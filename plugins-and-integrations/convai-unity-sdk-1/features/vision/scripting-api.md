...
### Scripting the Vision Pipeline

All Vision scripting centres on two MonoBehaviour-compatible types — `ConvaiVisionPublisher` for publish control and the frame source status interfaces for capture state. Domain events let you react to lifecycle changes without polling.

***

### ConvaiVisionPublisher

`ConvaiVisionPublisher` is a `MonoBehaviour` that manages the WebRTC video track. Obtain a reference with `GetComponent` or a serialised field.
...
| `FrameDimensions`      | `(int Width, int Height)` property | Output resolution. Returns `(0, 0)` before initialisation.                        |
...
{% hint style="warning" %}
`VisionFrameCaptured` fires every captured frame. At 15 fps over a 60-second session this is 900 events. Use `EventDeliveryPolicy.Immediate` and keep the handler lightweight. For analytics, consider sampling every N-th frame rather than subscribing to every event.
{% endhint %}
...
{% hint style="warning" %}
`IsVisionTrack` checks for the name `"vision"`, not `"unity-scene"`. With the default track name, `IsVisionTrack` returns `false`. Use `TrackName` directly to identify the track, or rename the track to `"vision"` in the Inspector if your integration depends on `IsVisionTrack`.
{% endhint %}
...
With scripting covered, see [Custom Frame Sources](/broken/pages/8b043e020fbbc0e70868e2e3994e8e6b2a7d4f3a) to publish any custom video pipeline, or [Troubleshooting & Diagnostics](/broken/pages/ce72ed1f95d78e73f4af5a61fbeaeb221e86e25d) if events are not firing as expected.
