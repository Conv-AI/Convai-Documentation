...
`VisionDebugPreview` is an Editor-only component that renders the live camera feed as a screen overlay and displays a capture statistics panel. It is automatically disabled in player builds, so it is safe to leave on prefabs and scenes.

Use it to answer **"What does the AI actually see?"** during development before shipping.
...
* On WebGL the overlay is blank because `ConvaiVisionPublisher` bypasses frame sources and uses `canvas.captureStream()` directly. There is no `RenderTexture` to display.
...
With the feed confirmed in the Editor, proceed to [Publishing & Policies](/broken/pages/1538f07b55ab5c719a79be190e1c775c62b5bd3c) to tune the transport budget, or [Troubleshooting & Diagnostics](/broken/pages/ce72ed1f95d78e73f4af5a61fbeaeb221e86e25d) if the overlay is blank.
