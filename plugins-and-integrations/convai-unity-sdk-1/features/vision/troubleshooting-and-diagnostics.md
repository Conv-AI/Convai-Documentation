...
Work through this page top to bottom. Most Vision failures fall into one of four categories: connection misconfiguration, frame source failure, platform-specific restrictions, or WebGL origin policy. Each section lists the symptom, likely cause, and fix.
...
Add `VisionDebugPreview` to any scene GameObject. Press Play. If the overlay shows a live image and `FPS > 0`, the feed is reaching the publisher. If the overlay is blank or FPS stays at zero, the frame source is not producing frames — continue to [Frame Source Issues](troubleshooting-and-diagnostics.md#frame-source-issues) below.
{% endstep %}
{% endstepper %}
...
| Debug overlay blank, FPS = 0 | Frame source in `Failed` state                | Check `ErrorKind` — see [Frame Source Issues](troubleshooting-and-diagnostics.md#frame-source-issues). |
| Feed is black                | Wrong `CameraCaptureMode` for render pipeline | See [Black Feed](troubleshooting-and-diagnostics.md#black-feed).                                       |
...
If `Auto` produces a black feed on URP/SRP, switch **Camera Capture Mode** to `ExplicitRenderCompatibility`.
...
If the issue is not covered here, check the domain events — subscribe to `VisionCaptureStopped` and `VideoTrackUnpublished` and log the `Reason` and `ErrorMessage` fields. See [Scripting API](/broken/pages/66ae09f082490e6e22eeabcf09762855075b37c0) for the event subscription pattern.
