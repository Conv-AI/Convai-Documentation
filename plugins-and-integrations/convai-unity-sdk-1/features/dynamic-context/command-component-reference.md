...
## ConvaiDynamicContextCommand Field Reference

`ConvaiDynamicContextCommand` is the no-code entry point for Dynamic Context. It is a `MonoBehaviour` that encapsulates one context operation and exposes its entire configuration through the Unity Inspector.

The component is marked `[DisallowMultipleComponent]` — Unity prevents adding a second instance to the same GameObject. To drive multiple commands from one NPC, place each additional command on a **child GameObject** (see [Multiple Commands Per NPC](command-component-reference.md#multiple-commands-per-npc)).
...
When `Execute()` is skipped due to a configuration or validation failure, the component logs a warning to the Unity Console prefixed with `[ConvaiDynamicContextCommand]`. The **On Execution Skipped** event also fires. Use the table below to identify and fix each warning.
...
See [Usage Examples](/broken/pages/1da079a97de2161cc4bfab80bf93ab3b6c189bc8) for complete end-to-end scenarios using each command type. For equivalent C# scripting control, see [Scripting API Reference](/broken/pages/92e033e69c53e803f7296bede3511ff924549b4d). For details on how updates are transmitted after `Execute()`, see [Sync Behavior and Timing](/broken/pages/fd07c33dbde4c8f46f1472ab841d25ba0bc5951e).
