...
| Lip sync                     | ✅ Full (see known issue below)                      |
| Actions                      | ✅ Full                                              |
...
### HTTPS and Browser Requirements
...
### Browser Audio Gesture Handling
...
The browser controls all microphone device selection. When conversation starts, the browser displays its own permission prompt and allows the user to select a microphone device.

**The SDK returns an empty device list on WebGL** — the Settings Panel microphone dropdown will show no entries. This is expected behavior, not an error. The browser's native permission UI handles device selection.

The microphone test functionality available on native platforms is not supported on WebGL.
...
{% hint style="warning" %}
WebGL Vision captures what the player sees in the browser — the game view. For scenarios where the character needs to see the learner's physical environment (webcam), use a desktop or mobile build with `WebcamVisionFrameSource` instead.
{% endhint %}
...
On WebGL, the SDK uses `RealtimePlaybackClock` (based on `Time.realtimeSinceStartupAsDouble`) instead of the hardware DSP clock used on native platforms. The DSP clock is tied to the audio hardware and provides sample-accurate timing. `Time.realtimeSinceStartupAsDouble` runs independently of the audio pipeline, which can cause visible desynchronization between speech audio and mouth animation — particularly on longer utterances.
...
#### Corporate Onboarding Training (WebGL with Explicit Start Button)

An enterprise L\&D team hosts a company policy training simulation on their corporate intranet. Employees access it via browser at `https://training.company.internal/onboarding`. A Convai character plays an HR representative who guides new hires through policy scenarios.
...
#### Embedded Safety Drill in an LMS (WebGL in iframe)
...
**Outcome:** The LMS page loads the simulation in the iframe. Operators click **Begin Drill**, grant microphone permission in the browser prompt, and begin the verbal compliance assessment. The LMS records completion via standard integration once the simulation reports its end state.
...
| Lip sync visibly drifts behind speech                         | Known defect: `RealtimePlaybackClock` used on WebGL instead of hardware DSP clock | No workaround. Account for this in QA and shipping criteria.                          |
