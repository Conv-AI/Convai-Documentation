...
Full field definitions, rig-specific guidance, and the pre-built `RealisticEmotionSlots` factory are covered in [Output Bindings](output-bindings.md).

## Conclusion

`ConvaiEmotionProfile` brings all smoothing, micro-burst, alternation, and output configuration into one portable asset that you can share across multiple NPC prefabs. Continue to [Output Bindings](output-bindings.md) to wire the smoothed scores to your character's face, or see [Emotion Taxonomy](emotion-taxonomy.md) to understand and customise the emotion vocabulary.
