...
* [Sync Behavior and Timing](sync-behavior-and-timing.md) — when and how each update type is transmitted, including the two-message Replace+Append pattern and the mechanics of the pre-conversation queue flush.

## Conclusion

`IConvaiDynamicContext` provides a precise, type-safe surface for every Dynamic Context operation. The tracked methods queue automatically before a conversation starts; `Apply()` does not — choose accordingly. For a complete breakdown of when and how each update type is transmitted, see [Sync Behavior and Timing](sync-behavior-and-timing.md).
